...
function checkProperty(obj, prop) {
  ...
  if (typeof obj !== "object" || obj === null) {
    console.error("Помилка: Перший параметр повинен бути об'єктом.");
    return false;
  }
  // Перевіряємо, чи вхідний параметр prop є рядком, якщо ні, повертаємо false
  if (typeof prop !== "string") {
    console.error("Помилка: Другий параметр повинен бути рядком.");
    return false;
  }
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (typeof obj1 !== 'object' || typeof obj2 !== 'object' || obj1 === null || obj2 === null) {
    return null; // Повертаємо null у випадку невірних вхідних параметрів
  }
  ...
  if (Object.isFrozen(obj1)) {
    ...
    return "Помилка: Об'єкт не було заморожено."; // Повертаємо повідомлення про помилку, якщо об'єкт не було "заморожено"
  }
...
function keysAndValues(obj) {
  ...
  if (typeof obj !== 'object' || obj === null) {
    return null; // Повертаємо null у випадку невірного вхідного параметра
  }
...
function filterObjectsByKey(arr, key) {
  ...
  if (!Array.isArray(arr)) {
    return []; // Повертаємо пустий масив у випадку невірного вхідного параметра
  }
...
function checkAndPreventExtensions(obj) {
  ...
  // Перевіряємо, чи об'єкт вже заморожений, якщо так, повертаємо false
...
function sealAndCheck(obj) {
  // Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо false
  if (typeof obj !== 'object' || obj === null) {
    return false; // Повертаємо false у випадку невірного вхідного параметра
  }
...
function checkOwnership(obj1, obj2, key) {
  ...
  if (obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key)) {
    // Повертаємо результат порівняння значень за допомогою строгого оператора порівняння
...
function convertArrayToObj(arr) {
  ...
  arr.forEach(([key, value]) => {  // Розпаковуємо підмасив за допомогою деструктурізації на окремі змінні для ключа та значення

    // Перевіряємо, чи існує вже ключ в об'єкті
    if (resultObj.hasOwnProperty(key)) {  // Перевіряємо, чи існує вже ключ в об'єкті,якщо так виводимо в консоль повідомлення `У масиві є дубльований ключ: ${key}`
      console.log(`У масиві є дубльований ключ: ${key}`);  // Додаємо ключ та значення до об'єкта
